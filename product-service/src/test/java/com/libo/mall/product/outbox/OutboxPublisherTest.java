...
class OutboxPublisherTest {
    ...
    void shouldClaimPublishAndMarkEventPublished() throws Exception {
        ...
        StockResultEvent payload = new StockResultEvent(
                UUID.randomUUID(), UUID.randomUUID(), Instant.now(), 10L, true, null
        );
        ...
        when(objectMapper.readValue(event.getPayload(), StockResultEvent.class)).thenReturn(payload);
        when(eventPublisher.publish(event.getTopic(), event.getMessageKey(), payload))
